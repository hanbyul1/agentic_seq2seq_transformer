...
- Post-deployment adaptation validation
- Prediction outputs
...
├── moe-swe-bench.py
│
├── post_deployment_humaneval_validation.py
├── post_deployment_swebench_validation.py
│
...
└── outputs/
    ...
    │   ├── after_adaptation/
    │   ├── metrics.json
    │   ├── before_adaptation.zip
    │   └── after_adaptation.zip
    │
    ├── post_deployment_swebench/
    │   ├── before_adaptation/
    │   ├── after_adaptation/
    │   ├── metrics.json
    │   ├── before_adaptation.zip
...
- localized post-deployment adaptation,
- improved maintainability,
- reduced retraining costs.

The proposed architecture employs fixed agent roles instead of dynamically activated experts and evaluates post-deployment specialization through before/after adaptation validation experiments.
...
- prediction records,
- post-deployment adaptation artifacts.
...
The repository includes dedicated validation scripts for evaluating localized specialization after deployment.
...
Evaluates specialization of the Specification Agent by comparing outputs before and after adaptation.
...
Evaluates specialization of the Issue Analysis Agent by comparing outputs before and after adaptation.

---

## Output Artifacts

The `outputs/` directory contains generated predictions, validation artifacts, adaptation results, and deployed checkpoints.

### Post-Deployment HumanEval Validation

```text
outputs/post_deployment_humaneval/
```

Contains:

- pre-adaptation outputs,
- post-adaptation outputs,
- adaptation metrics,
- archived result packages.

These artifacts are used to evaluate specialization of the Specification Agent after deployment.

### Post-Deployment SWE-bench Validation

```text
outputs/post_deployment_swebench/
```
...
These artifacts are used to evaluate specialization of the Issue Analysis Agent after deployment.
...
Contains prediction artifacts generated by the MoE baseline on HumanEval.
...
Contains prediction artifacts generated by the MoE baseline on SWE-bench.

### Deployed Checkpoints

```text
outputs/swebench/checkpoints/
```

Contains deployed model checkpoints used during post-deployment adaptation experiments.
...
- adaptation efficiency,
- post-deployment adaptation results.
...
**To what extent can agentic modeling enable localized post-deployment adaptation while reducing adaptation cost and improving output quality?**
...
Benchmark Evaluation
      │
      ▼
Deployment Checkpoint Creation
      │
      ▼
